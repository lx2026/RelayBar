...
- Record process completion from `terminationHandler` and consult that record, not `isRunning`, before escalating.
- Skip the escalation entirely once the process is known to have exited.
- Keep the two-second delay and the single-escalation guarantee for a process that genuinely ignores `SIGTERM`.

## Acceptance

- A process that exits after `terminate()` is never sent `SIGKILL`.
