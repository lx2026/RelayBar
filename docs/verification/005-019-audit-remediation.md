...
- `swift test` passed 152 tests with 11 opt-in tests skipped and no failures, up from 133 before the change. With `RELAYBAR_LOOPBACK_SSH_DIR` set, the two live tests also pass against a real OpenSSH server.
...
- **Task 008.** `testCancellationStopsTheProcessAndRemovesPartial` records a cooperative child's signals and verifies `SIGTERM` is delivered without the delayed `SIGKILL`. `testCancellationForceStopsAProcessThatIgnoresTermination` verifies a stubborn child receives exactly one `SIGTERM` and one `SIGKILL`, the continuation resolves as cancellation, and partial data is removed. Both paths passed 30 consecutive final iterations. `testBatchInputDescriptorSuppressesSIGPIPE` pins the descriptor option that keeps an early child exit from terminating RelayBar. `testSpawnInheritsBatchInputWhenPipeReaderIsStandardInput` closes the test process's descriptor zero, proves the batch reader takes that descriptor, and verifies the spawned child still receives its input under close-by-default spawning.
...
- PID registration, exit-source setup, and callback installation happen in one critical section, so cancellation cannot reap the child before its continuation has a completion path.
- The first pull-request CI run for Task 008 exited with signal 13 when `/bin/echo` closed the batch-input pipe before RelayBar wrote its command. Local shells happened to ignore `SIGPIPE`, so repeated local cancellation tests could not reveal it. The parent writer now uses `F_SETNOSIGPIPE`, turning that platform-dependent process termination into the `EPIPE` error the existing write path already handles.
- `POSIX_SPAWN_CLOEXEC_DEFAULT` closes every descriptor not named by a file action. RelayBar therefore adds the standard-input `dup2` action even when the batch reader is already descriptor zero; the descriptor-zero regression test exercises that otherwise rare GUI-process state.
